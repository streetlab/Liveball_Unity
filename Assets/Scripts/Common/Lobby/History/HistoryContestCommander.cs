		...
		Debug.Log ("UserMgr.ContestStatus : " + UserMgr.ContestStatus );
			...
			//	CreatItem ();
				...
				for (int i = 0; i<SeqList.Count; i++) {
					...
					Debug.Log("List [SeqList [i] [a]].contestName : " + List [SeqList [i] [a]].contestName);
					...
					Item2.transform.FindChild ("BG").FindChild ("rewordscore").GetComponent<UILabel> ().text = List [SeqList [i] [a]].rewardScore.ToString ();
					Debug.Log("List [SeqList [i] [a]].rewordCount.ToString () : " + List [SeqList [i] [a]].rewardCount.ToString ());
					Item2.transform.FindChild ("BG").FindChild ("rewordcount").GetComponent<UILabel> ().text = List [SeqList [i] [a]].rewardCount.ToString ();
