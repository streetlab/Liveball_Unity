...
public class OkButten : MonoBehaviour {
	...
	public void Ok(){
		if (name == "NO") {
			...
			transform.parent.parent.FindChild ("ProfileSetting").GetComponent<ProfileSetting> ().SetMemberName(transform.parent.parent.FindChild ("SetNamePage").FindChild("Input").FindChild("Label").GetComponent<UILabel>().text);
			}else{
